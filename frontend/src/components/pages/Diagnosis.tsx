...
const Diagnosis = () => {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      const analyzeData: { message: string; imageUrl: string; prediction: string } =
      ...
      // filter โรคที่ตรงกับ prediction
      const disease = allData.find((d) => d.name === analyzeData.prediction);
        ...
        <div className="w-full max-w-3xl flex flex-col gap-4">
          ...
          <Slide direction="up">
            <div className="bg-zinc-800 p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow">
              ...
              <p>
                <strong>โรค:</strong> {result.diseaseName}
              </p>
              <p>
                <strong>สาเหตุ:</strong> {result.cause}
              </p>
              <p>
                <strong>การดูแลรักษา:</strong> {result.treatment}
              </p>
            </div>
          </Slide>
        </div>
